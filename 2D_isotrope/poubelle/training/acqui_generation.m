...
function []= acqui_generation(z_sources,x_sources,y_sources,z_recepts,x_recepts,y_recepts, nz, nx, ny,h)

nb_sources = length(z_sources);
nb_recepts = length(z_recepts);
 
	if ( max(z_sources)>nz*h || max(x_sources)>nx*h || max(y_sources)>ny*h || max(z_recepts)>nz*h || max(x_recepts)>nx*h || max(y_recepts)>ny*h )
